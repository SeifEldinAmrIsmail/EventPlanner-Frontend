...
export class EventService {
  ...
  /**
   * IMPORTANT:
   * Base is "/api/events" (no trailing slash)
   * For create (POST), backend expects "/events/" so we must call "/api/events/"
   * to avoid FastAPI redirect dropping "/api".
   */
  private base = `${environment.apiUrl}/events`; // => /api/events
  ...
  /**
   * Backend you shared does NOT show an invite endpoint.
   * Keep this only if you really have it in backend.
   */
  invite(id: string, email: string): Observable<{ ok: true }> {
    return this.http.post<{ ok: true }>(`${this.base}/${encodeURIComponent(id)}/invite`, { email });
  }

  /** Attendee sets status (FIXED PATH: respond not response) */
